...
public class SettingsMenu extends Menu {
    ...
    public void handleMenu(InventoryClickEvent inventoryClickEvent) {
        ...
        if(inventoryClickEvent.getCurrentItem().getType() == Material.BOOK) {
            inventoryClickEvent.getView().getTopInventory().close();
            new MessageSetsSettingsMenu(playerMenuUtility.getOwner()).open();
        }
        ...
        if(inventoryClickEvent.getCurrentItem().getType() == Material.SANDSTONE || inventoryClickEvent.getCurrentItem().getType() == Material.STONE || inventoryClickEvent.getCurrentItem().getType() == Material.DIAMOND_BLOCK) {
            inventoryClickEvent.getView().getTopInventory().close();
            new BlocksSettingsMenu(playerMenuUtility.getOwner()).open();
        }
        ...
        if (inventoryClickEvent.getCurrentItem().getType() == Material.STICK || inventoryClickEvent.getCurrentItem().getType() == Material.BLAZE_ROD || inventoryClickEvent.getCurrentItem().getType() == Material.BONE) {
            inventoryClickEvent.getView().getTopInventory().close();
            new SticksSettingsMenu(playerMenuUtility.getOwner()).open();
        }
...
//            inventory.setItem(9, messageSet3);
//        }
//
...
//        inventory.setItem(5, stick2);
        ...
        ItemStack mats = makeItem(Material.getMaterial(resultInvSort.getString("block_material").toUpperCase()), I18NUtils.get(playerMenuUtility.getOwner(), "blocks"));
        inventory.setItem(9+2, mats);

        ItemStack sticks = makeItem(Material.getMaterial(resultInvSort.getString("stick_material").toUpperCase()), I18NUtils.get(playerMenuUtility.getOwner(), "stick"));
        inventory.setItem(9+4, sticks);

        ItemStack messageSets = makeItem(Material.BOOK, I18NUtils.get(playerMenuUtility.getOwner(), "messages"));
        inventory.setItem(9+6, messageSets);
